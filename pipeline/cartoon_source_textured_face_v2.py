...
CONTRACT_RELATIVE_PATH = "concept/characters/june_oxley_phase34_source_textured_visemes_v2.json"
IMPLEMENTATION_RELATIVE_PATH = "pipeline/cartoon_source_textured_face_v2.py"
EXPECTED_CONTRACT_CANONICAL_SHA256 = "c23d572d994fd72a837b57850eb4ee1c49b3a33189e0764bc705c7d0f209ec11"
...
class FrameEvidence:
    ...
    upper_dentition_forbidden_source_pixels: int
    f_visible_incisor_area: int
    f_lower_lip_contact_length: int
    f_cavity_height: int
    h_tongue_tip_area: int
    h_tongue_groove_pixels: int
    ...
    layer_write_counts: dict[str, int]
...
def _require_equal(actual: Any, expected: Any, label: str) -> None:
    if actual != expected:
        raise SourceTexturedFaceError(f"complete Phase34 v2 contract required: {label}: {actual!r} != {expected!r}")
...
def _validate_contract(contract: dict[str, Any]) -> None:
    ...
    _require_equal(contract["contract_id"], "june_oxley_phase34_source_textured_visemes_v2", "contract id")
    _require_equal(contract["phase"], "phase34_source_textured_viseme_deformation_blink_fix", "phase")
    ...
    _require_equal(
        representation["layer_write_alpha_threshold_u8"],
        LAYER_WRITE_ALPHA_THRESHOLD_U8, "layer write alpha threshold",
    )
    _require_equal(
    ...
    _require_equal(
        representation["blink_interpolation"],
        "explicit_linear_perceptual_closure_table_v1", "blink interpolation",
    )
    ...
    _require_equal(contract["performance"]["blink_closure_by_frame"], {
        "4": 0.0, "5": 0.25, "6": 0.5, "7": 0.75, "8": 1.0,
        "9": 0.75, "10": 0.5, "11": 0.25, "12": 0.0,
    }, "explicit blink closure table")
...
def blink_closure(contract: dict[str, Any], frame_number: int) -> float:
    table = contract["performance"]["blink_closure_by_frame"]
    return float(table.get(str(frame_number), 0.0))
...
def _record_layer(coverage: np.ndarray, mask: np.ndarray, layer_name: str) -> None:
    coverage[np.asarray(mask) >= LAYER_WRITE_ALPHA_THRESHOLD_U8] |= np.uint16(LAYER_BITS[layer_name])
...
def _compose_authored_oral_anatomy(
    ...
    oral_write = oral_alpha_u8 >= LAYER_WRITE_ALPHA_THRESHOLD_U8
    ...
    f_weight = float(pose_weights.get("F", 0.0))
    g_weight = float(pose_weights.get("G", 0.0))
    fg_bridge = float(np.clip(4.0 * f_weight * g_weight, 0.0, 1.0))
    h_weight = float(pose_weights.get("H", 0.0))
    tongue_tip_shape = (
        ((mouth_u / max(width * 0.29, 1.0)) ** 2
         + ((mouth_v - opening * 0.16) / max(opening * 0.24, 1.0)) ** 2 <= 1.0)
        & cavity_hard
        & (mouth_v >= -opening * 0.02)
    )
    tongue_tip_alpha = np.clip(
        _soft(tongue_tip_shape.astype(np.uint8) * 255, 0.65).astype(np.float32)
        * h_weight * oral_activation * 0.88,
        0, 255,
    ).astype(np.uint8)
    tongue_material = np.clip(
        oral_rgb.astype(np.float32) * np.asarray([0.82, 0.70, 0.72], dtype=np.float32)
        + np.asarray([38.0, 8.0, 12.0], dtype=np.float32),
        0, 255,
    ).astype(np.uint8)
    _blend(canvas, tongue_material, tongue_tip_alpha)
    tongue_tip_write = tongue_tip_alpha >= LAYER_WRITE_ALPHA_THRESHOLD_U8
    _record_layer(coverage, tongue_tip_alpha, "oral_interior")
    owner[tongue_tip_write] = 4
    tongue_groove = (
        tongue_tip_shape
        & (np.abs(mouth_u + 0.8) <= 1.5)
        & (mouth_v >= opening * 0.05)
    )
    groove_alpha = np.clip(
        _soft(tongue_groove.astype(np.uint8) * 255, 0.45).astype(np.float32)
        * h_weight * oral_activation * 0.68,
        0, 255,
    ).astype(np.uint8)
    groove_color = np.zeros_like(canvas)
    groove_color[:] = np.asarray([78, 25, 31], dtype=np.uint8)
    _blend(canvas, groove_color, groove_alpha)
    _record_layer(coverage, groove_alpha, "oral_interior")
    owner[groove_alpha >= LAYER_WRITE_ALPHA_THRESHOLD_U8] = 4
    ...
    upper_write = upper_alpha_u8 >= LAYER_WRITE_ALPHA_THRESHOLD_U8
    ...
    tongue = (
        visible & (red > 105.0) & (green > 34.0) & (blue > 35.0)
        & (red > green * 1.50) & (mouth_v > -opening * 0.05)
    ) | (tongue_tip_alpha > 72)
    ...
    f_dental_window = (
        (np.abs(mouth_u) <= width * 0.28)
        & (mouth_v >= -opening * 0.30)
        & (mouth_v <= opening * 0.12)
        & cavity_hard
    )
    source_feather_alpha = np.clip(
        source_feather_alpha.astype(np.float32)
        * (1.0 - f_dental_window.astype(np.float32) * f_weight * 0.88)
        * (1.0 - 0.45 * fg_bridge),
        0, 255,
    ).astype(np.uint8)
    ...
    source_feather_write = source_feather_alpha >= LAYER_WRITE_ALPHA_THRESHOLD_U8
    source_feather_front = source_feather_alpha > 24
    _record_layer(coverage, source_feather_alpha, "source_lip_edge")
    owner[source_feather_write] = 7
    ...
    f_window_suppression = 1.0 - f_dental_window.astype(np.float32) * f_weight * 0.92
    bridge_reveal = 1.0 - 0.35 * fg_bridge
    upper_alpha = np.clip(
        upper_alpha.astype(np.float32) * f_window_suppression * bridge_reveal, 0, 255,
    ).astype(np.uint8)
    lower_alpha = np.clip(
        lower_alpha.astype(np.float32) * f_window_suppression * bridge_reveal, 0, 255,
    ).astype(np.uint8)
    ...
    f_visible_incisor = upper_teeth & ~f_contact_write
    f_tooth_contact = f_contact_write & (
        cv2.dilate(upper_teeth.astype(np.uint8), cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (5, 5))) > 0
    )
    f_contact_columns = int(np.unique(np.where(f_tooth_contact)[1]).size) if f_tooth_contact.any() else 0
    ...
    texture_values = cv2.cvtColor(warped_source, cv2.COLOR_RGB2GRAY)[texture_ring]
    cavity_y = np.where(cavity_hard)[0]
    cavity_height = int(cavity_y.max() - cavity_y.min() + 1) if cavity_y.size else 0
    return {
        "cavity": int(cavity_hard.sum()),
        ...
        "f_visible_incisor_area": int(f_visible_incisor.sum()) if f_weight > 0.5 else 0,
        "f_lower_lip_contact_length": f_contact_columns if f_weight > 0.5 else 0,
        "f_cavity_height": cavity_height if f_weight > 0.5 else 0,
...
def _native_frame(
    ...
    oral = {
        ...
        "f_visible_incisor_area": 0,
        "f_lower_lip_contact_length": 0,
        "f_cavity_height": 0,
        "h_tongue_tip_area": 0,
        "h_tongue_groove_pixels": 0,
        "h_tongue_lower_lip_occlusion_pixels": 0,
    ...
    write_counts = {
        name: int(((coverage & bit) != 0).sum())
        for name, bit in LAYER_BITS.items()
        if int(((coverage & bit) != 0).sum()) > 0
    }
    evidence = FrameEvidence(
        ...
        f_visible_incisor_area=int(oral["f_visible_incisor_area"]),
        f_lower_lip_contact_length=int(oral["f_lower_lip_contact_length"]),
        f_cavity_height=int(oral["f_cavity_height"]),
        h_tongue_tip_area=int(oral["h_tongue_tip_area"]),
        h_tongue_groove_pixels=int(oral["h_tongue_groove_pixels"]),
        h_tongue_lower_lip_occlusion_pixels=int(oral["h_tongue_lower_lip_occlusion_pixels"]),
        ...
        layer_write_counts=write_counts,
...
def _preflight(prepared: PreparedSourceTexturedFace) -> dict[str, Any]:
    ...
    for first, second in (("A", "F"), ("C", "E"), ("B", "D"), ("G", "H"), ("F", "X")):
    ...
    return {
        ...
        "F_visible_incisor_area": non_neutral["F"].f_visible_incisor_area,
        "F_lower_lip_contact_length": non_neutral["F"].f_lower_lip_contact_length,
        "F_cavity_height": non_neutral["F"].f_cavity_height,
        "H_tongue_tip_area": non_neutral["H"].h_tongue_tip_area,
        "H_tongue_groove_pixels": non_neutral["H"].h_tongue_groove_pixels,
        "H_tongue_lower_lip_occlusion_pixels": non_neutral["H"].h_tongue_lower_lip_occlusion_pixels,
...
def _preflight_gates(contract: dict[str, Any], metrics: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    checks = [
        ...
        ("F_visible_incisor_area", metrics["F_visible_incisor_area"], ">=", thresholds["minimum_F_visible_incisor_area"]),
        ("F_lower_lip_contact_length", metrics["F_lower_lip_contact_length"], ">=", thresholds["minimum_F_lower_lip_contact_length"]),
        ("F_cavity_height", metrics["F_cavity_height"], ">=", thresholds["minimum_F_cavity_height"]),
        ("H_tongue_tip_area", metrics["H_tongue_tip_area"], ">=", thresholds["minimum_H_tongue_tip_area"]),
        ("H_tongue_groove_pixels", metrics["H_tongue_groove_pixels"], ">=", thresholds["minimum_H_tongue_groove_pixels"]),
        ("H_tongue_lower_lip_occlusion", metrics["H_tongue_lower_lip_occlusion_pixels"], ">=", thresholds["minimum_H_tongue_lower_lip_occlusion_pixels"]),
...
def _preview_output_path(contract: dict[str, Any], development_label: str | None) -> Path:
    ...
    if development_label != "candidate-09":
        raise SourceTexturedFaceError("Phase34 v2 preview label is pinned to candidate-09")
    return output.with_name(f"{output.name}-{development_label}")


def _candidate09_output_evidence(
    frames: list[Image.Image], contract: dict[str, Any],
) -> tuple[dict[str, Any], list[dict[str, Any]]]:
    if len(frames) != 96:
        raise SourceTexturedFaceError(f"Candidate09 output evidence requires 96 frames, got {len(frames)}")
    manifest_path = _resolve_repo_path(contract["locks"]["candidate08_manifest"]["path"])
    candidate08_manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
    expected_entries = candidate08_manifest.get("frames", [])
    if [entry.get("frame") for entry in expected_entries] != list(range(1, 97)):
        raise SourceTexturedFaceError("Candidate08 provenance manifest does not bind ordered frames 1 through 96")

    arrays = [np.asarray(frame, dtype=np.uint8) for frame in frames]
    current_entries = [
        {"frame": index, "rgb_sha256": _raw_frame_hash(array)}
        for index, array in enumerate(arrays, start=1)
    ]
    preserved = sum(
        current["rgb_sha256"] == previous["rgb_sha256"]
        for current, previous in zip(current_entries, expected_entries)
    )
    changed_frames = [
        current["frame"] for current, previous in zip(current_entries, expected_entries)
        if current["rgb_sha256"] != previous["rgb_sha256"]
    ]

    roi = contract["preencode_gates"]["output_face_roi_xyxy"]
    adjacent: list[float] = []
    for first, second in zip(arrays, arrays[1:]):
        first_face = first[roi[1]:roi[3], roi[0]:roi[2]]
        second_face = second[roi[1]:roi[3], roi[0]:roi[2]]
        delta = np.abs(first_face.astype(np.float32) - second_face.astype(np.float32)).mean(axis=2)
        adjacent.append(float(cv2.boxFilter(delta, -1, (8, 8), normalize=True).max()))
    maximum_adjacent = max(adjacent)
    maximum_adjacent_index = adjacent.index(maximum_adjacent)
    blink_values = adjacent[3:11]
    maximum_blink = max(blink_values)
    maximum_blink_index = blink_values.index(maximum_blink) + 3

    clock = contract["clock"]
    native_eye_support = np.zeros((clock["source_height"], clock["source_width"]), dtype=np.uint8)
    for eye_name in ("viewer_left_eye", "viewer_right_eye"):
        eye = contract["semantic_geometry_native_xy"][eye_name]
        native_eye_support = cv2.bitwise_or(
            native_eye_support,
            phase33._ellipse_mask(
                native_eye_support.shape,
                tuple(eye["center"]),
                (eye["radius"][0] + 7, eye["radius"][1] + 7),
            ),
        )
    output_eye_support = cv2.resize(
        native_eye_support,
        (clock["output_width"], clock["output_height"]),
        interpolation=cv2.INTER_NEAREST,
    ) > 0

    archive_path = _resolve_repo_path(contract["locks"]["candidate08_archive"]["path"])
    ...
    with gzip.open(archive_path, "rb") as archive:
        ...
        if header != expected_header:
            raise SourceTexturedFaceError(f"Candidate08 archive header changed: {header}")
        previous = np.zeros((1080, 1920, 3), dtype=np.uint8)
        for frame_number, current in enumerate(arrays, start=1):
            payload = archive.read(6220800)
            if len(payload) != 6220800:
                raise SourceTexturedFaceError(f"Candidate08 archive frame {frame_number} is truncated")
            delta = np.frombuffer(payload, dtype=np.uint8).reshape((1080, 1920, 3))
            candidate08 = np.bitwise_xor(delta, previous)
            if frame_number in changed_frames:
                changed = np.any(current != candidate08, axis=2)
                changed_counts[str(frame_number)] = int(changed.sum())
                maximum_outside = max(maximum_outside, int((changed & ~output_eye_support).sum()))
            previous = candidate08
        if archive.read(1):
            raise SourceTexturedFaceError("Candidate08 archive has trailing payload")

    metrics = {
        "candidate08_preserved_frame_hashes": preserved,
        "candidate08_changed_frames": changed_frames,
        "candidate08_changed_pixel_counts": changed_counts,
        "maximum_candidate08_changed_pixels_outside_eye_support": maximum_outside,
        "maximum_output_adjacent_face_8x8_mean_delta": maximum_adjacent,
        "maximum_output_adjacent_face_8x8_mean_delta_frame_pair": [
            maximum_adjacent_index + 1, maximum_adjacent_index + 2,
        ],
        "maximum_output_blink_adjacent_face_8x8_mean_delta": maximum_blink,
        "maximum_output_blink_adjacent_face_8x8_mean_delta_frame_pair": [
            maximum_blink_index + 1, maximum_blink_index + 2,
        ],
        "output_frames_evaluated": len(frames),
    }
    thresholds = contract["preencode_gates"]
    checks = [
        ("candidate08_preserved_frame_hashes", preserved, "==", thresholds["required_candidate08_preserved_frame_hashes"]),
        ("candidate08_changed_frames", changed_frames, "==", thresholds["required_candidate08_changed_frames"]),
        ("candidate08_changed_pixels_outside_eye_support", maximum_outside, "==", thresholds["required_candidate08_changed_pixels_outside_eye_support"]),
        ("output_adjacent_face_8x8_mean_delta", maximum_adjacent, "<=", thresholds["maximum_output_adjacent_face_8x8_mean_delta"]),
        ("output_blink_adjacent_face_8x8_mean_delta", maximum_blink, "<=", thresholds["maximum_output_blink_adjacent_face_8x8_mean_delta"]),
    ]
    gates = []
    for name, actual, operator, threshold in checks:
...
def write_unencoded_preview(
    ...
    *,
    ...
    try:
        ...
        output_metrics, output_gates = _candidate09_output_evidence(frames, prepared.contract)
        prepared.preflight_measurements.update(output_metrics)
        all_preflight_gates = _preflight_gates(prepared.contract, prepared.preflight_measurements) + output_gates
        failures = [gate for gate in all_preflight_gates if gate["passed"] is not True]
        if failures:
            raise SourceTexturedFaceError("Candidate09 output preflight failed: " + json.dumps(failures, sort_keys=True))
        ...
        manifest = {
            ...
            "clock": prepared.contract["clock"],
            "candidate08_provenance": {
                name: prepared.contract["locks"][name]
                for name in (
                    "candidate08_manifest", "candidate08_archive", "candidate08_renderer",
                    "candidate08_attempt_report", "candidate08_attempt_video",
                )
            },
            ...
            "preflight_gates": all_preflight_gates,
            "preflight_gate_count": len(all_preflight_gates),
            "preflight_gates_passed": sum(gate["passed"] is True for gate in all_preflight_gates),
            "preflight_gates_failed": sum(gate["passed"] is not True for gate in all_preflight_gates),
        ...
        return {
            ...
            "manifest_raw_sha256": _sha256(output / manifest_path.name),
            "manifest_lf_normalized_sha256": _sha256_lf_normalized_text(output / manifest_path.name),
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Render the Phase34 v2 Candidate09 blink-corrected preview")
...
def main() -> None:
    args = _parse_args()
    if not args.preview:
        raise SourceTexturedFaceError("Phase34 v2 is preview-only until a new bound visual review authorizes a separate encode")
