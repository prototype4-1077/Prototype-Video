...
REFERENCE_OPUS_SHA256 = "6c4aa23dd75bc18127a724bd9a7f506ef0aefec9252ad63ae10e6a4b31503a7d"
...
def _restore_exact_voice(build_dir: Path) -> None:
    ...
    if target.exists():
        try:
            ...
            if target_digest in {EXACT_VOICE_SHA256, REFERENCE_OPUS_SHA256}:
                return
        except OSError:
            pass

    reference_parts = sorted(build_dir.glob("voice_reference.opus.b64.part*"))
    if reference_parts:
        for raw in _decode_part_set(reference_parts):
            digest = hashlib.sha256(raw).hexdigest()
            if digest != REFERENCE_OPUS_SHA256:
                continue
            tmp = build_dir / ".vo.mp3.reference.tmp"
            tmp.write_bytes(raw)
            os.replace(tmp, target)
            print(
                f"restored exact supplied voice performance from Opus transfer: "
                f"{len(raw)} bytes sha256={digest}",
                flush=True,
            )
            return
        raise RuntimeError(
            "Supplied voice reference parts failed checksum validation; "
            f"expected sha256={REFERENCE_OPUS_SHA256}"
        )
    ...
    names = ", ".join(path.name for path in reference_parts + exact_parts + legacy)
    raise RuntimeError(
